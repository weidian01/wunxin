...
//console.log(data);
    ...
    for (var i in data) {
        ...
        html += '<div class="cart-goodsname"><a href="'+wx.productURL(data[i].pid)+'" title="'+data[i].pname+'" target="_blank">'+data[i].pname.substring(0,25)+'</a><br/>' +
            '<span class="font5">￥'+wx.fPrice(data[i].sell_price)+'</span>';
        html += '<span> &nbsp;&nbsp;&nbsp; 尺码：'+data[i].product_size+'</span>';
        html += '<span> &nbsp;&nbsp;&nbsp; 数量：'+parseInt(data[i].num)+'</span>';
        html += '</div><div class="clear" onclick="cart.deleteCartItem('+i+')"></div>';
        html += '</div>';
        totalPrice += parseInt(data[i].final_price);
...
//console.log(cartData.length);
    for (var ci in cartData) {
        ...
        totalPrice += cartData[ci]['final_price'];
    }
...
wx.advert = function(position_id)
{
    ...
    if (!wx.isEmpty(data) || !wx.isEmpty(data['advert'])) {
        return;
    }
